...
$PASSWORD = '$2y$10$ECK/VS5uKZ70ukoiTwOGRuCOreRY8kOU/02ZOFxrvsf1ux838z6hq';

error_reporting(0);
$VERSION = "8.22";
$HOST = "https://fileadmin-backend.simsv.com";
$LOGIN = false;
...
case "stylesheet": header("content-type: text/css"); ?> /* <style> */
	...
	*:not(.textEditorContainer *)::-webkit-scrollbar{display:none;}
	...
	input:not(.textEditorContainer input),select,textarea,#totpForm{font-size:.95rem;padding:10px;width:100%;border-radius:5px;border:1px solid #DADADA;background:#F9F9FB;transition:border .2s,background .2s;margin:15px 0;caret-color:#1E9FFF;}
	input:not(.textEditorContainer input):focus,select:hover,textarea:focus,#totpForm:focus-within{background:white;border:1px solid #1E9FFF;}
...
case "javascript": header("content-type: text/javascript"); ?> /* <script> */
	...
	function init(silent){
		Globals = {
			...
			extRuntime: {version: 3, editorStatusBar: [], customFileMenu: [], blockEditorLoad: 0, textEditorLoadTries: 0},
	...
	let tabData = {
		...
		openWith: {
			...
			initFunction(tabId, file) {
				...
				if (handlers.firstHandler) getTabContent(tabId, ".container").innerHTML += `<div onclick="selectHandler('${handlers.firstHandler}', this)" ondblclick="confirmHandler()"><font>&#x${fileHandlers[handlers.firstHandler].icon};</font><span>${fileHandlers[handlers.firstHandler].uiName}<small>${getConfig("defaultFileHandler")[getFileExt(file)] != handlers.firstHandler ? "最适合处理此类文件" : "已设为默认打开方式"}</small></span></div>`;
				...
				if (handlers.fullSupportedList.length) {
					...
					if (!handlers.fileSupportedList.length) setTimeout(() => { getTabContent(tabId, ".container>a").click(); }, 1);
				}
	...
	function handleTabsKeydown() {
		if (event.key == "w" && event.ctrlKey) { event.preventDefault(); closeTab(Globals.currentTab); }
	}
	...
	function handleFilesKeydown() {
		if (event.key != "v") event.preventDefault();
